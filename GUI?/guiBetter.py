...
class DataModel():
    ...
    def __init__(self, filename):
        ...
        try:
            ...
            def price_collation(string1, string2):
                # Create a dictionary with the sort order for each character
                sort_order = {
                    '€': 1,
                    '-': -2,
                    ' ': 0,
                ...
                # Break each string down into a list of characters
                char_list1 = list(string1)
                char_list2 = list(string2)

                # Compare the characters in each list, starting from the first character
                for i in range(min(len(char_list1), len(char_list2))):
                    # If the characters are different, compare their numeric values
                    if char_list1[i] != char_list2[i]:
                        return sort_order[char_list1[i]] < sort_order[char_list2[i]]

                # If all the characters are the same up to this point, the shorter string comes first
                return len(char_list1) < len(char_list2)

            print("Επιτυχής σύνδεση στη βάση δεδομένων", filename)
            sqlite_select_Query = "select sqlite_version();"
            self.cursor.execute(sqlite_select_Query)
            record = self.cursor.fetchall()
            self.con.create_collation('custom', price_collation)
            self.con.execute("drop index if exists bus_price_range_idx ;")
            self.con.execute("CREATE INDEX if not exists bus_price_range_idx ON Business (price_range COLLATE custom);")
            for rec in record:
